...
class Comando(commands.Cog):
    ...
    async def dev(self, ctx):
        embed=discord.Embed(title="RM Yuki", url="https://images4.alphacoders.com/909/thumb-1920-909912.png", description=f"Olá {ctx.author.name} sou o Zury, sou desenvolvedor de bots para discord. Tenho um servidor onde comercializo bots, caso tenha interesse, entre em contato.", color=0x11ff00)
    ...
    async def anuncio(self, ctx):
        ...
        embed=discord.Embed(title=":space_invader:Nova Atualização RM Yuki!", colour=discord.Colour(0x00ff11), description="Nova atualização RM Yuki!\nConfira agora o que veio de novo e fique por dentro!")
        embed.add_field(name="Info", value="Ver informações sobre desenvolvedor e artista das imagens.", inline=False)
        embed.add_field(name="Retirada de fotos da betoneira", value="Foi retirada fotos da betoneira para não expor sua imagem.", inline=False)
        embed.add_field(name="mock", value="Comando mock tem como função gerar um texto ou palavra aleatória para zuar um membro", inline=False)
