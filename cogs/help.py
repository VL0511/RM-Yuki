...
class Help(commands.Cog):
    ...
    async def help(self, ctx):
        embed = discord.Embed(icon_url="https://artfiles.alphacoders.com/136/thumb-136777.jpg",
                              title=":space_invader: Ajuda Yuki",
                              description=f"Olá {ctx.author.mention} sou o mini-Yuki, tenho apenas 16 anos, sou apenas mais um bot do discord. Tenho funções para anti-sapm, interação e limpar chats.",
                              color=0x00ff11)
        embed.add_field(name="📋 Veja nossa lista de comandos",
                        value="Consulte nossas lista de comandos utilizando o comando 'cmd'.", inline=False)
        embed.add_field(name="💻Deseja ver informações sobre desenvolvedor?", value="Utilize o comando 'dev'.",
                        inline=False)
        embed.add_field(name=":robot:Ver informações sobre o bot", value="Utilize o comando 'info'.", inline=False)
        embed.set_footer(
            text="Lista de comandos para interação do servidor caso tenha alguma duvida, chame um administrador para tirar suas dúvidas.")
        await ctx.send(embed=embed)

    @commands.Cog.listener()
    async def on_message(self, message):
        if (f'<@{str(self.client.user.id)}>' == message.content) or (
                f'<@!{str(self.client.user.id)}>' == message.content):
            ctx = await self.client.get_context(message)
            await self.client.get_command('help')(ctx)

    @commands.command()
    async def cmd(self, ctx):
        embed = discord.Embed(title=":space_invader: Ajuda Yuki", colour=discord.Colour(0x00ff11),
